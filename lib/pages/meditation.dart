...
const _patternDefault =
    _BreathPattern('Default (4–4)', [ _BreathPhase(_PhaseKind.inhale, 4), _BreathPhase(_PhaseKind.exhale, 4) ], 5);

const _patternBox =
    _BreathPattern('Box (4–4–4–4)', [ _BreathPhase(_PhaseKind.inhale, 4), _BreathPhase(_PhaseKind.hold, 4), _BreathPhase(_PhaseKind.exhale, 4), _BreathPhase(_PhaseKind.hold, 4) ], 5);

const _pattern478 =
    _BreathPattern('4–7–8', [ _BreathPhase(_PhaseKind.inhale, 4), _BreathPhase(_PhaseKind.hold, 7), _BreathPhase(_PhaseKind.exhale, 8) ], 4);

const _patternResonant =
    _BreathPattern('Resonant (~6/min)', [ _BreathPhase(_PhaseKind.inhale, 5), _BreathPhase(_PhaseKind.exhale, 5) ], 5);

const _patternSigh =
    _BreathPattern('Physiological Sigh', [ _BreathPhase(_PhaseKind.inhale, 2), _BreathPhase(_PhaseKind.inhale, 1), _BreathPhase(_PhaseKind.exhale, 6) ], 2);

// ---- New longer & variety patterns (some 15 minutes) ----
const _patternBox15 =
    _BreathPattern('Box (4–4–4–4) · 15m', [ _BreathPhase(_PhaseKind.inhale, 4), _BreathPhase(_PhaseKind.hold, 4), _BreathPhase(_PhaseKind.exhale, 4), _BreathPhase(_PhaseKind.hold, 4) ], 15);

const _patternResonant15 =
    _BreathPattern('Resonant (5–5) · 15m', [ _BreathPhase(_PhaseKind.inhale, 5), _BreathPhase(_PhaseKind.exhale, 5) ], 15);

const _patternEqual66 =
    _BreathPattern('Equal (6–6)', [ _BreathPhase(_PhaseKind.inhale, 6), _BreathPhase(_PhaseKind.exhale, 6) ], 10);

const _patternTriangle =
    _BreathPattern('Triangle (4–4–6)', [ _BreathPhase(_PhaseKind.inhale, 4), _BreathPhase(_PhaseKind.hold, 4), _BreathPhase(_PhaseKind.exhale, 6) ], 10);
...
class _MeditationPageState extends State<MeditationPage> with SingleTickerProviderStateMixin {
  ...
  late final AnimationController _breathCtrl =
      AnimationController(vsync: this, duration: Duration(seconds: _pattern.cycleSeconds));
  ...
  void _applyPattern(_BreathPattern p, {bool start = true}) {
    _pattern = p;
    _breathCtrl.stop();
    _breathCtrl.duration = Duration(seconds: _pattern.cycleSeconds);
    _breathCtrl.reset();
    _resetToMinutes(p.suggestedMinutes);
    if (start) _start();            // will call repeat(); otherwise stays static
    setState(() {});
  }

  /// Determine current phase and subprogress within that phase (0..1)
  ({_BreathPhase phase, double subProgress, _PhaseKind lastNonHold, int phaseIndex})
    ...
    for (final ph in _pattern.phases) {
      if (t < ph.seconds) {
        ...
        return (phase: ph, subProgress: sub, lastNonHold: lastNonHold, phaseIndex: idx);
      }
    ...
    return (phase: ph, subProgress: 1.0, lastNonHold: lastNonHold, phaseIndex: _pattern.phases.length - 1);
  }
  ...
  // --- Haptics ---
  void _buzzForPhase(_PhaseKind k) {
    // Lightweight, distinct cues. (Built-in taps; no extra dependency.)
    switch (k) {
      case _PhaseKind.inhale:
        HapticFeedback.lightImpact();   // start inhale
        break;
      case _PhaseKind.hold:
        HapticFeedback.selectionClick(); // transition into hold
        break;
      case _PhaseKind.exhale:
        HapticFeedback.mediumImpact();  // start exhale
        break;
    }
  }
        ...
        child: SafeArea(
          ...
          child: ListView(
            ...
            children: [
                          ...
                          children: [
                            Transform.scale(
                              ...
                              child: Container(
                                ...
                                child: Center(
                                  child: Text(
                                    _cueText(), // changes only while running
              ...
              Container(
                ...
                child: const Column(
                  ...
                  children: [
                    ...
                    Text('• If thoughts wander, gently return to the breath.'),
                  ],
                ),
              ),
              ...
              const Text('Guided breathing exercises',
              ...
              _ExerciseCard(
                ...
                bullets: const ['Inhale 4s', 'Hold 7s', 'Exhale 8s (slow)', 'Repeat 4–6 cycles'],
              ...
              _ExerciseCard(
                ...
                tag: 'Calm · 5–10 min',
                bullets: const ['Inhale 5s', 'Exhale 5s', 'Keep breaths smooth and even'],
              ...
              _ExerciseCard(
                ...
                bullets: const ['Inhale 4s', 'Hold 4s', 'Exhale 4s', 'Hold 4s · repeat for full session'],
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            child: Text(tag, style: const TextStyle(fontSize: 12, color: Color(0xFF0D7C66))),
          ),
