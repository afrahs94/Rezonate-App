...
class _ScramblePageState extends State<ScramblePage> {
  ...
  void _new() {
    ...
    do { scrambled.shuffle(rnd); } while (scrambled.join() == target);
    _start = DateTime.now();
    ctrl.clear();
    setState(() {});
  }
    ...
    if (ctrl.text.trim().toUpperCase() == target) {
      ...
      await showDialog(
        ...
        builder: (_) => AlertDialog(
          ...
          content: Text('You solved "${target.toUpperCase()}" in ${secs}s'),
  ...
  Widget build(BuildContext context) {
    return _GameScaffold(
      ...
      topBar: Row(
        children: [
          OutlinedButton.icon(onPressed: _new, icon: const Icon(Icons.shuffle_rounded, size: 18), label: const Text('New')),
        ],
      ),
      child: Column(
        children: [
          Wrap(
            ...
            children: scrambled.map((ch) => Container(
              ...
              child: Text(ch, style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 18)),
            )).toList(),
          ),
          ...
          TextField(
            ...
            decoration: InputDecoration(
              ...
              border: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: const BorderSide(color: _ink)),
            ),
            onSubmitted: (_) => _check(),
          ),
          const SizedBox(height: 8),
          FilledButton.icon(onPressed: _check, icon: const Icon(Icons.check_rounded, size: 18), label: const Text('Check')),
        ],
      ),
    );
  }
